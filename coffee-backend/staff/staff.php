<?php
    ...
    $staff_query = mysqli_query($conn, "SELECT linked_store_id FROM users WHERE user_id = $staff_id");
    $staff_row = mysqli_fetch_assoc($staff_query);
    $staff_store_id = $staff_row['linked_store_id'];
    ...
    // --- PRODUCTS: only beans stocked in this staff's store ---
    $bean_query = "SELECT cb.*, p.province_name, si.quantity_kg
                   FROM coffee_bean cb
                   JOIN province p ON cb.origin_province_id = p.province_id
                   JOIN store_inventory si ON si.bean_id = cb.bean_id
                   WHERE si.store_id = $staff_store_id
                   $bean_search_clause
                   $filter_clause";

    $result_bean = mysqli_query($conn, $bean_query);

    // --- ORDER LOGS: only orders from this staff's store ---
    $order_sort   = $_GET['order_sort'] ?? 'newest';
    $order_status = $_GET['order_status'] ?? 'all';

    $order_where = "WHERE s.store_id = $staff_store_id";
    ...
    $store_name_query = mysqli_query($conn, "SELECT store_name FROM store WHERE store_id = $staff_store_id");
    $store_name_row = mysqli_fetch_assoc($store_name_query);
    $store_name = $store_name_row['store_name'] ?? 'Your Branch';
?>
<!DOCTYPE html>
<html>
    <head>
    ...
    <body>
        ...
        <header>
            <nav>
                <!-- Static store display — staff can only see their own branch -->
                <span style="font-family:Montserrat,sans-serif; font-weight:600; color:#5D372A;">
        ...
        <h2>Staff — <?= htmlspecialchars($store_name) ?></h2>
        ...
        <div class="table-container">
            ...
            <div id="product-management" class="product-management">
                <div class="tab-controls">
                    ...
                    <form method="GET" action="staff.php" style="display:inline-flex; gap:12px; align-items:center; flex-wrap:wrap;">
                        ...
                        <?php if (!empty($bean_search)): ?>
                            <a href="staff.php?tab=product-management" style="color:#EA672D; font-size:13px; font-weight:600;">✕ Clear</a>
                        <?php endif; ?>
                ...
                <table class="viewrecords">
                    ...
                    <?php if (mysqli_num_rows($result_bean) == 0): ?>
                        <tr><td colspan="8" style="text-align:center; padding:30px; color:#888;">No beans found for your branch.</td></tr>
                    <?php else: ?>
            ...
            <div id="order-management" class="order-management">
                ...
                <table class="viewrecords">
                    ...
                    <?php if (mysqli_num_rows($result_orderlogs) == 0): ?>
                        <tr>
                            <td colspan="9" style="text-align:center; padding:30px; color:#888;">No orders for your branch yet.</td>
                        </tr>
                    <?php else: ?>
    ...
    <script>
        ...
        async function viewOrder(saleId) {
            ...
            content.innerHTML = `
                ...
                <table class="viewrecords">
                    <tr><th>Bean</th><th>Quantity</th><th>Unit Price</th><th>Subtotal</th></tr>
                    ${itemsHTML}
                    <tr>
